    ...
    public function send() {
        ...
        if (!empty($this->text)) {
            if (empty($this->attachments) && empty($this->html)) {
                ...
                $letter .= 'Content-Transfer-Encoding: base64'.self::EOL.self::EOL;
            }
            $letter .= chunk_split(base64_encode($this->text)).self::EOL.self::EOL;
        }
        ...
        if (!empty($this->html)) {
            ...
            $letter .= chunk_split(base64_encode($this->html)).self::EOL.self::EOL;
        }
