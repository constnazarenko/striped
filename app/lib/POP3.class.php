...
class POP3
{
    ...
    */
     ...
     */
    public function __construct( $strLogFile = NULL, $bAPOPAutoDetect = TRUE, $bHideUsernameAtLog = TRUE, $strEncryption = 'tcp', $bUseSockets = TRUE )
    {
    ...
    public function connect( $strHostname , $intPort = 110, $arrConnectionTimeout = array("sec" => 10, "usec" => 0) ,$bIPv6 = FALSE )
    {
        ...
        */
        ...
        else
        {
            ...
            $errno = 0;
            $errstr = '';
            if( !$this->resSocket = @@@fsockopen($this->strProtocol. "://" . $this->strHostname .":". $this->intPort, $errno, $errstr, $dTimeout) )
            {
                return false;
                //throw new POP3_Exception( "[". $errno."] -- ". $errstr, self::ERR_STREAM );
            }
    ...
    private function openlog()
    {
        if( !$this->bLogOpened && is_writeable($this->strLogFile) )
        {
            ...
            if( !$this->resLogFp = fopen($this->strLogFile, (filesize($this->strLogFile) > 1024 * 1024) ? 'w' : 'a') )
            {
